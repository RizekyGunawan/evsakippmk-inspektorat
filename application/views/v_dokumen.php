            ...
            <div class="card">
             <div class="card-header d-flex p-0 col-md-12">
              <ul class="nav nav-pills ml-3 p-3">
                ...
                <button type="button" class="btn btn-primary" <?php if($load['id_dokumen'] == ""){echo "";} elseif($load['id_dokumen'] != ""){echo "disabled";}  else {echo "disabled";}; ?>
                      data-toggle="modal" data-target="<?php if($load['id_dokumen'] == ""){echo "#TambahData";} elseif($load['id_dokumen'] != ""){echo "#";}  else {echo "#";}; ?>">
            Tambah Data
        </button>
                    <?php endforeach; ?>
                    <?php endif; ?>
                ...
                <button type="button" class="btn btn-primary" <?php if($loadall['id_dokumen'] == ""){echo "";} elseif($loadall['id_dokumen'] != ""){echo "disabled";}  else {echo "disabled";}; ?>
                      data-toggle="modal" data-target="<?php if($loadall['id_dokumen'] == ""){echo "#TambahDataAll";} elseif($loadall['id_dokumen'] != ""){echo "#";}  else {echo "#";}; ?>">
              ...
              <div class="card-body table-responsive">
                <table id="table" style="width: 100%;" class="table table-bordered">
                  ...
                  <td class="text-center"><?php 
						       if ($unt['status_data'] == "0"){echo "Draft";} elseif ($unt['status_data'] == "1"){echo "Final";} else {echo "";}; ?></td>

                        <td class="text-center align-middle" style="width: 30px">
                          <?php if ($unt['status_data'] == "0" && ((($this->session->userdata('id_role') == 1 || $this->session->userdata('id_role') == 5) && ($this->session->userdata('id_unit') == $this->session->userdata('id_unit2') || 
                               $this->session->userdata('id_unit') == $this->session->userdata('id_unit_es1'))) || $this->session->userdata('id_role') == 4)): ?>
                          <?php if ($unt['dok_lap'] != ""): ?>
                          <div class= "btn btn-success btn-xs" data-toggle="modal" data-target="#EditData<?php echo $unt['id_dokumen']; ?>"><i class="fas fa-edit"></i></div>
                       <?php endif; ?>
                        <?php endif; ?>
                     </td>
...
<!-- Modal Tambah All -->
<div class="modal fade" id="TambahDataAll" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-sm" role="document">
    <div class="modal-content">
      <div class="modal-header">
...
foreach ($unit3 as $unt) : 
    ...
    foreach ($rows_jawaban0 as $row) {
        ...
        switch ($row->jawaban0) {
            ...
            case '90':
            case '80':
                if ($row->jawabanantara !== 'BB') $has_mismatch = true;
                break;
            case '70':
                if ($row->jawabanantara !== 'B') $has_mismatch = true;
                break;
            case '60':
                if ($row->jawabanantara !== 'CC') $has_mismatch = true;
                break;
            case '50':
                if ($row->jawabanantara !== 'C') $has_mismatch = true;
                break;
            case '30':
                if ($row->jawabanantara !== 'D') $has_mismatch = true;
                break;
            case '0':
                if ($row->jawabanantara !== 'E') $has_mismatch = true;
                break;
        }
        if ($has_mismatch) break;
    }
?>
<div class="modal fade" id="EditData<?php echo $unt['id_dokumen']; ?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-sm" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <?php if ($has_empty_jawaban0 || $has_empty_jawaban1 || $has_mismatch) : ?>
                    <div class="alert alert-danger text-justify">
                        <?php if ($has_empty_jawaban0 || $has_empty_jawaban1) : ?>
                           <i class="fas fa-exclamation-triangle"></i> Masih terdapat jawaban pada Subkomponen dan/atau Kriteria yang belum terisi. Harap lengkapi pengisian jawaban sebelum mengubah status ke Final.
                           <br>
                        <?php endif; ?>
                        <?php if ($has_mismatch) : ?>
                            <i class="fas fa-exclamation-triangle"></i> Masih terdapat Jawaban Akhir pada Subkomponen yang tidak sesuai dengan Jawaban Antara. Harap periksa dan perbaiki jawaban tersebut.
                        <?php endif; ?>
                    </div>
                <?php else : ?>
...
<?php $no = 0; 
foreach ($unit3 as $unt) : ?>
<div class="modal fade" id="EditDatalap<?php echo $unt['id_dokumen']; ?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
