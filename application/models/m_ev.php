...
class M_ev extends CI_Model
{
	...
	public function insert_ev($tahun, $id_unit)
	{
        ...
        AND NOT EXISTS (
            SELECT 1 FROM ta_ev0 exist 
            WHERE exist.tahun = a.tahun AND exist.id_unit = a.id_unit 
            AND exist.id_pm0 = a.id_pm0
        )");
        ...
        a.tahun = $tahun and a.id_unit = $id_unit
        AND NOT EXISTS (
            SELECT 1 FROM ta_ev exist 
            WHERE exist.tahun = a.tahun AND exist.id_unit = a.id_unit 
            AND exist.id_pm = a.id_pm
        )");
	...
	public function get_rekap_all_units($tahun)
	{
		...
		FROM ref_unit d 
		LEFT JOIN ta_ev0 b ON d.id_unit=b.id_unit AND b.tahun = '$tahun'
		LEFT JOIN ref_komponen a ON b.id_komponen=a.id_komponen 
		LEFT JOIN ref_subkomponen c ON b.id_subkomponen=c.id_subkomponen 
		WHERE d.id_unit > 0
		GROUP BY d.id_unit, d.nm_unit
		ORDER BY d.id_unit ASC");
		return $query->result_array();
	}
