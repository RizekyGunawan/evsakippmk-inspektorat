...
class M_dok_ev extends CI_Model
{
	...
	{
		$query = $this->db->query("SELECT * from ta_dok_ev a  inner join ref_unit b on a.id_unit=b.id_unit where a.tahun = '$tahun' ORDER BY a.id_unit ASC ");
		return $query->result_array();
	}
	...
	public function get_data3monev2($tahun)
	{
		...
		$query = $this->db->query("SELECT a.*,b.persen FROM (SELECT a.*,b.nm_unit, b.kd_unit, b.id_unit as sort_id_unit, ROUND(sum((CASE 
			...
			(SELECT id_dok_ev, SUM(CASE WHEN jawaban2 IS NOT NULL AND jawaban2 <> '' THEN 1 ELSE 0 END) / COUNT(*) * 100 AS persen from ta_ev where tahun = $tahun GROUP BY id_dok_ev) b on a.id_dok_ev=b.id_dok_ev ORDER BY a.sort_id_unit ASC ");
		return $query->result_array();
	}
